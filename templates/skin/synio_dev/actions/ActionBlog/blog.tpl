...
<div class="nav-menu-wrapper">
	<div class="menu_blog_content">
		...
		<div {if $sMenuSubItemSelect=='new'}class="active"{else}class="inactive"{/if}><a href="{$sMenuSubBlogUrl}newall/">{$aLang.blog_menu_collective_new}</a>{if $iCountTopicsBlogNew>0} <a href="{$sMenuSubBlogUrl}new/" class="new">+{$iCountTopicsBlogNew}</a>{/if}</div>
	...
	{if $sPeriodSelectCurrent}
		<ul class="nav nav-pills nav-pills-dropdown blog_tpl">
